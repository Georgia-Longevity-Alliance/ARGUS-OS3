...
ARGUS-OS3 строит **прогениторные карты** на основе spindle orientation pedigrees центриолей (OS1) и доказательства конвергенции (OS2).

> **🔴 Зависит от успеха OS2.** Без доказательства что spindle orientation pedigree → судьба, карты строить не из чего.
...
А ГРАФ (узлы = генетические сети, рёбра = spindle orientation pedigree)
```

| Элемент графа | Что это |
|--------------|---------|
| **Узел** | Паттерн генной экспрессии (генетическая сеть) |
| **Ребро** | Spindle orientation pedigreeия центриоли (∥/⟂ или θ) |
| **Конвергенция** | Разные spindle orientation pedigree → один узел |
...
| Данные | Источник |
|--------|----------|
| Spindle orientation pedigreeии центриолей (∥/⟂, θ) | ARGUS-OS1 |
...
| Шаг | Действие |
|:---:|----------|
| 1 | Получить spindle orientation pedigree + судьбы от OS1 |
...
| 2 | ARGUS-OS1/CONCEPT.md | Spindle orientation pedigreeии центриолей |
